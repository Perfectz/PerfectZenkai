...
export function Sheet({ open, onOpenChange: _onOpenChange, children }: SheetProps) {
...
export const SheetContent: React.FC<SheetContentProps> = ({ side = 'bottom', className, ...props }) => {
  const basePosition =
    side === 'bottom'
      ? 'inset-x-0 bottom-0'
      : side === 'top'
  ...
  return (
    <div
      className={cn(
        'fixed z-50 bg-background shadow-lg transition-transform animate-in slide-in-from-'+side+'-5',
        basePosition,
        className
      )}
      {...props}
